...
# Provides direct link for script executions
...
case "$1" in
  webserver)
    ...
    sleep 20
    # Manually add connections from AIRFLOW_CONN environment variables
    # See https://github.com/puckel/docker-airflow/issues/441
    while IFS='=' read -r name value ; do
      if [[ $name == 'AIRFLOW_CONN'* ]]; then
        CONN_ID=$(echo $name | sed -e 's/^AIRFLOW_CONN_//' | tr '[:upper:]' '[:lower:]')
        CONN_URI=${value}
        echo "Registering conn_id=$CONN_ID"
        airflow connections -a --conn_id $CONN_ID --conn_uri $CONN_URI
      fi
    done < <(env)
    # Creates an ADMIN default user when authentication is provided
    if [ $AIRFLOW__WEBSERVER__AUTHENTICATE = True ]; then
      airflow create_user -r Admin -u $AIRFLOW_ADMIN_USERNAME -e $AIRFLOW_ADMIN_EMAIL -f $AIRFLOW_ADMIN_FIRSTNAME -l $AIRFLOW_ADMIN_LASTNAME -p $AIRFLOW_ADMIN_PASSWORD
    fi
